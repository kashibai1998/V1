...
public class SupplierController {
	...
	@PostMapping("/add")
	@ResponseBody
	public ResponseEntity<?> addSupplier(@RequestBody SupplierDto supply) {
		ResponseEntity<?> response;
		try {
			response = new ResponseEntity<SupplierDto>(suppliermanager.saveSupplier(supply), HttpStatus.OK);
	...
	@PostMapping("/add/internal/")
	@ResponseBody
	public ResponseEntity<?> addInternalSupplier(@RequestBody InternalSupplierDto internalSupplierDto) {
		ResponseEntity<?> response;
		try {
			response = new ResponseEntity<InternalSupplierDto>(
					suppliermanager.saveInternalSupplier(internalSupplierDto), HttpStatus.OK);
		} catch (InventoryException e) {
			response = new ResponseEntity(e.getMessage(), HttpStatus.OK);
			e.printStackTrace();
		}
	...
	@ResponseBody
	public ResponseEntity<?> allProducts(@PathVariable(value = "id") UUID id) {
		ResponseEntity<?> response;
		try {
			response = new ResponseEntity(suppliermanager.getProducts(id), HttpStatus.OK);
		} catch (InventoryException e) {
			response = new ResponseEntity(e.getMessage(), HttpStatus.OK);
			e.printStackTrace();
		}
